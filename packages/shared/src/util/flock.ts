...
export namespace Flock {
  ...
  async function stale(lockDir: string, heartbeatPath: string, metaPath: string, staleMs: number) {
    ...
    if (heartbeat) return now - heartbeat.mtimeMs > staleMs

    const meta = await stats(metaPath)
    if (meta) return now - meta.mtimeMs > staleMs
  ...
  async function acquireLockDir(lockDir: string, input: { key: string; onWait?: Wait; signal?: AbortSignal }, opts: Opts) {
