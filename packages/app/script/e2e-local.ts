...
const sandbox = await fs.mkdtemp(path.join(os.tmpdir(), "folonite-e2e-"))
const keepSandbox = process.env.FOLONITE_E2E_KEEP_SANDBOX === "1"

const serverEnv = {
  ...process.env,
  FOLONITE_DISABLE_SHARE: process.env.FOLONITE_DISABLE_SHARE ?? "true",
  FOLONITE_DISABLE_LSP_DOWNLOAD: "true",
  FOLONITE_DISABLE_DEFAULT_PLUGINS: "true",
  FOLONITE_EXPERIMENTAL_DISABLE_FILEWATCHER: "true",
  FOLONITE_TEST_HOME: path.join(sandbox, "home"),
  XDG_DATA_HOME: path.join(sandbox, "share"),
  XDG_CACHE_HOME: path.join(sandbox, "cache"),
  XDG_CONFIG_HOME: path.join(sandbox, "config"),
  XDG_STATE_HOME: path.join(sandbox, "state"),
  FOLONITE_E2E_PROJECT_DIR: repoDir,
  FOLONITE_E2E_SESSION_TITLE: "E2E Session",
  FOLONITE_E2E_MESSAGE: "Seeded for UI e2e",
  FOLONITE_E2E_MODEL: process.env.FOLONITE_E2E_MODEL ?? "opencode/gpt-5-nano",
  FOLONITE_CLIENT: "app",
...
const runnerEnv = {
  ...
  VITE_FOLONITE_SERVER_HOST: "127.0.0.1",
  VITE_FOLONITE_SERVER_PORT: String(serverPort),
...
try {
  ...
  if (seedExit !== 0) {
    ...
    console.log(`folonite server listening on http://127.0.0.1:${serverPort}`)
