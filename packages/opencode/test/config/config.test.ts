...
const managedConfigDir = process.env.FOLONITE_TEST_MANAGED_CONFIG_DIR!
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["FOLONITE_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:FOLONITE_CONSOLE_TOKEN}" } } },
        }),
      ),
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["FOLONITE_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["FOLONITE_CONSOLE_TOKEN"]
    }
  }
})
...
test("does not try to install dependencies in read-only FOLONITE_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.FOLONITE_CONFIG_DIR
  process.env.FOLONITE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.FOLONITE_CONFIG_DIR
    else process.env.FOLONITE_CONFIG_DIR = prev
  }
})

it.live(
  "installs dependencies in writable FOLONITE_CONFIG_DIR",
...
// Note: preload.ts sets FOLONITE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
describe("FOLONITE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["FOLONITE_DISABLE_PROJECT_CONFIG"]
    process.env["FOLONITE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["FOLONITE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["FOLONITE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["FOLONITE_DISABLE_PROJECT_CONFIG"]
    ...
    try {
      ...
      if (originalDisable === undefined) {
        ...
        process.env["FOLONITE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
  ...
  test("FOLONITE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["FOLONITE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["FOLONITE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["FOLONITE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["FOLONITE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from FOLONITE_CONFIG_DIR, not project
...
describe("FOLONITE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in FOLONITE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["FOLONITE_CONFIG_CONTENT"]
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["FOLONITE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["FOLONITE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in FOLONITE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["FOLONITE_CONFIG_CONTENT"]
  ...
  test("resolves relative plugin paths in FOLONITE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["FOLONITE_CONFIG_CONTENT"]
