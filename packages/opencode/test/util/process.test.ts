...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.FOLONITE_TEST ?? "")'), {
      env: {
        FOLONITE_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "FOLONITE_TEST_SHELL"], {
      shell: true,
      env: {
        FOLONITE_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("FOLONITE_TEST_SHELL=ok")
  })
