...
import { sanitizedProcessEnv } from "@opencode-ai/core/util/folonite-process"
...
export interface SearchResult {
  ...
  partial: boolean
  /**
   * When `partial` is true, distinguishes between a hard pattern parse error
   * (`"invalid_pattern"`) and other partial-read situations such as
   * unreadable paths (`"partial_io"`). Lets callers report a specific error
   * for invalid patterns without falsely flagging a search that just hit
...
function searchArgs(input: SearchInput) {
  ...
  // Check for undefined explicitly so an explicit `limit: 0` (caller asking
  // for a hard zero-result cap) is forwarded as `--max-count=0` instead of
  // being silently dropped to an unbounded search.
  if (input.limit !== undefined) args.push(`--max-count=${input.limit}`)
    ...
    Effect.gen(function* () {
      ...
      const filepath = yield* Effect.cached(
        Effect.gen(function* () {
          ...
          // Let real PlatformError surface through the declared error channel
          // of files()/search()/tree() instead of escaping as a defect — an
          // unreadable cached binary or unwritable bin dir should be a normal
          // failure callers can catch, not a process-level crash.
          if (system && (yield* fs.isFile(system))) return system
          ...
          yield* fs.ensureDir(Global.Path.bin)
      ...
      const check = Effect.fnUntraced(function* (cwd: string) {
        // Don't `orDie` here — real FS errors (permission denied, IO failures)
        // should flow through the declared PlatformError | Error channel of
        // files/search/tree instead of becoming defects. Only synthesize an
        // ENOENT when isDir cleanly returns false.
        if (yield* fs.isDir(cwd)) return
      ...
      const search: Interface["search"] = Effect.fn("Ripgrep.search")(function* (input: SearchInput) {
        ...
        const program = Effect.scoped(
          Effect.gen(function* () {
            ...
            // Detect invalid-pattern errors specifically by sniffing stderr.
            // ripgrep emits "regex parse error" / "error parsing pattern" /
            // "Syntax(" markers when the pattern itself is malformed; any
            // other code-2 exit (unreadable paths, IO errors during walk)
            // is partial_io and should not be reported as "invalid pattern".
            const isInvalidPattern =
              code === 2 &&
              (stderr.includes("regex parse error") ||
                stderr.includes("error parsing pattern") ||
                stderr.includes("Syntax("))
            return {
              items: code === 1 ? [] : items,
              partial: code === 2,
              ...(code === 2 && { partialReason: isInvalidPattern ? "invalid_pattern" : "partial_io" }),
