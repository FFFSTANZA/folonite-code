    ...
    await bootstrap(process.cwd(), async () => {
      const files = await AppRuntime.runPromise(
        Effect.gen(function* () {
          ...
          if (queryLower) stream = stream.pipe(Stream.filter((file) => file.toLowerCase().includes(queryLower)))
...
const SearchCommand = cmd({
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      // Print the full result (items + partial + partialReason) so debug
      // sessions can tell invalid-pattern and partial-IO cases apart from a
      // clean empty match list.
