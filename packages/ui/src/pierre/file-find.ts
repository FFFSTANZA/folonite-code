...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("folonite-find")
  api.delete("folonite-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("folonite-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("folonite-find", new Highlight(...rest))
    return true
  }
